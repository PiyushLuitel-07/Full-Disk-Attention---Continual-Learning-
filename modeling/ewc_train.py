...
STAGE_DIRECTORY = PROJECT_ROOT / "data_labeling" / "data_labels" / "continual_stages_simplified_labels"
...
def main():
    ...
    print(f"Stages discovered: {[stage['number'] for stage in stages]}")
    ...
    model = Attn_Net(im_size=IMAGE_SIZE, num_classes=2, attention=True, init="kaimingUniform").to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        holdout_loss, holdout_accuracy = evaluate(model, loaders["holdout"], criterion, device)

        print(
            f"Epoch {epoch}/{EPOCHS} | "
            f"train loss={train_loss:.4f} | train accuracy={train_accuracy:.4f} | "
            f"holdout loss={holdout_loss:.4f} | holdout accuracy={holdout_accuracy:.4f}"
        )
